...
-->";
    ...
    /// </summary>
    public const string ENDTAG_PATTERN =
        @"</(?<EndTagName>\w+?(?::\w+?)?(?:\.\w+?)*?)
(?:\s+(?:\w+?(?::\w+?)?=""[^><]*?""))*
\s*?>";
    ...
    /// </summary>
    public const string STARTTAG_PATTERN =
        @"<(?<StartTagName>[\w:.]+?)(?=[\s/>])
(?:\s+(?<AttribName>[\w\.]+?(?::\w+?)?="".*?""))*
\s*?/?>";
...
-->";
        ...
        this TextSelection selection)
    {
        ...
        Tuple<XamlNode, XamlNode> firstAndLast =
            _rootNode.GetFirstLastNodesBetweenPoints(
        ...
        {
            if (firstAndLast.Item1.IsNodeWithin(origEndPoints.TopPoint - 1,
                                                origEndPoints.BottomPoint - 1)
        ...
        this TextSelection selection)
    {
        ...
        if (ep.IsEmpty)
        {
            ...
            return selection.SelectNode() ? WiddenSelectionResult.Success :
                   WiddenSelectionResult.NodeSelectError;
        }
                                      ...
                                      string name = "")
    {
        ...
        return name == string.Empty
               || Regex.IsMatch(selection.Text,
                                string.Format(@"<(?<StartTagName>{0})(?=[\s/>])", name));
    }
    ...
    private static EditorPoints GetEditorPoints(Tuple<XamlNode, XamlNode>
            firstAndLast)
    {
        EditorPoints ep = EditorPoints.GetEditorPoints(firstAndLast.Item1.TopPoint
                          + 1,
